...
public class Linear {
     ...
     *</pre>
     */
    private static void solve_l2r_l1l2_svc(Problem prob, double[] w, double eps, double[] CW, SolverType solver_type) {
        int l = prob.l;
        int w_size = prob.n;
        int i, s, iter = 0;
        double C, d, G;
        ...
        double upper_bound[] = new double[l];// {Double.POSITIVE_INFINITY, 0, Double.POSITIVE_INFINITY};
        for (i = 0; i < l; i++) {
     ...
     */
    public static Model train(Parameters params, Data data, Problem prob, Parameter param) {
        ...
        if (param.solverType == SolverType.L2R_L2LOSS_SVR || //
            ...
            double[] weighted_C = new double[prob.l];
            for (int i = 0; i < prob.l; i++){
                weighted_C[i] = param.C;
                if(params.getUseWeighting()){
                	weighted_C[i] *= data.getLabelImportance(prob.labels[i]);
                }
            }
            ...
            if (param.solverType == SolverType.MCSVM_CS) {
                ...
                if (nr_class == 2) {
                    ...
                    train_one(sub_prob, param, model.w, weighted_C);
                } else {
                	assert false;
                    ...
                    for (int i = 0; i < nr_class; i++) {
                        ...
                        train_one(sub_prob, param, w, weighted_C);
    ...
    private static void train_one(Problem prob, Parameter param,
			double[] w, double[] weighted_C) {
        solve_l2r_l1l2_svc(prob, w, param.getEps(), weighted_C, SolverType.L2R_L2LOSS_SVC_DUAL);
	}
