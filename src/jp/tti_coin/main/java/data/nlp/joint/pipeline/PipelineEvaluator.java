...
import data.nlp.joint.Word;
import data.nlp.joint.WordLabelUnit;
import eval.Evaluator;

public class PipelineEvaluator extends Evaluator {

	public PipelineEvaluator(Parameters params, Inference infer) {
		super(params, infer);
	}
	...
	public void evaluate(Model nerModel, Model relModel, Data valid){
		if(params.getVerbosity() == 0)return;
		evaluateAll(nerModel, relModel, valid);
	}
	
	private void getEntities(Multimap<String, Offset> entities, Map<String, Tuple<String, Offset>> idEntityMap, Instance instance, Label label){
	...
	public void evaluateAll(Model nerModel, Model relModel, Data valid){
		int ws_correct = 0;
		int wt_sum = 0;
		...
		for(int idx = 0;idx < valid.size();idx++){
			...
			List<State> yStarStates = ((PipelineInference)getInference()).inferBestState(nerModel, relModel, instance, true, false);
				...
				for(Instance instance:document.getInstances()){
					...
					for(int i = 0;i < instance.size();i++){
						...
						if(!yStarStates.get(0).getLabel().getLabel(i).isNegative()){
							...
							String[] labels = labelUnit.getLabel().split("\\|");
							Node w1, w2;
							w1 = pair.getW1().getWord();
							w2 = pair.getW2().getWord();
							for(int j = 0;j < labels.length;j++){
								if(labels[j].isEmpty())continue;
								relationId++;
								String[] label = labels[j].split(":");
